...
export async function squareGet(path) {
  ...
  if (Array.isArray(payload.errors) && payload.errors.length > 0) {
    ...
    if (/AUTH|ACCESS|PERMISSION|FORBIDDEN|UNAUTHORIZED/i.test(code)) {
      throw new SquareError(`Square denied access to payout data (${code}). Check the Production Access Token and Payouts permission in Square.`, 409);
    }
    throw new SquareError(`Square returned a payout API error (${code}). No accounts have been changed.`, 409);
  }
  return payload;
}
